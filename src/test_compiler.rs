...
use crate::code::Opcode::{
    OpAdd, OpArray, OpBang, OpCall, OpConstant, OpDiv, OpEq, OpGreaterThan, OpHash, OpIndex,
    OpJump, OpJumpNotTrue, OpMinus, OpMul, OpNotEq, OpNull, OpPop, OpSetGlobal, OpSub, OpTrue,
};
...
fn test_instructions(actual: Instructions, expected: Vec<Instructions>) {
    ...
    assert_eq!(
        ...
        "wrong length got {} wanted {}",
        actual,
        expected
    );
    for (actual_byte, expected_byte) in actual
        .content
        .clone()
        .into_iter()
        .zip(expected.content.clone())
    {
        assert_eq!(
            actual_byte, expected_byte,
            "wrong byte got {} wanted {}",
            actual, expected
        );
    }
}
    ...
    let tests = vec![
        ...
        CompilerTestCase {
            ...
            expected_constants: vec![
                ...
                Object::IntegerObject(20),
                Object::IntegerObject(3333),
            ],
            expected_instructions: vec![
...
fn test_string_expressions() {
    let tests = vec![
        ...
        CompilerTestCase {
            ...
            expected_constants: vec![
                Object::StringObject("mon".to_string()),
                Object::StringObject("key".to_string()),
            ],
            expected_instructions: vec![
                make(OpConstant, vec![0]).unwrap(),
                make(OpConstant, vec![1]).unwrap(),
...
fn test_functions() {
    let inputs = vec![
        ...
        CompilerTestCase {
            input: "fn() { }".to_string(),
            expected_constants: {
                vec![Object::CompiledFunction(CompiledFunctionStruct {
                    instructions: join_instructions(vec![
...
fn test_function_calls() {
    let tests = vec![
        ...
        CompilerTestCase {
            ...
            expected_constants: vec![Object::CompiledFunction(CompiledFunctionStruct {
                instructions: join_instructions(vec![
    ...
    let tests = vec![
        CompilerTestCase {
            input: "len([]);
                    push([], 1);"
                .to_string(),
            expected_constants: vec![IntegerObject(1)],
            expected_instructions: vec![
                ...
                Object::CompiledFunction(
                    CompiledFunctionStruct{
                        instructions: join_instructions(
                            vec![
                                make(code::Opcode::OpGetFree, vec![0]).unwrap(),
                                make(code::Opcode::OpGetLocal, vec![0]).unwrap(),
                                make(OpAdd, vec![]).unwrap(),
                                make(code::Opcode::OpReturnValue, vec![]).unwrap()
                            ]
                ...
                Object::CompiledFunction(
                    CompiledFunctionStruct{
                        instructions: join_instructions(
                            vec![
                                make(code::Opcode::OpGetLocal, vec![0]).unwrap(),
                                make(code::Opcode::OpClosure, vec![0, 1]).unwrap(),
                                make(code::Opcode::OpReturnValue, vec![]).unwrap()
                            ]
                        ),
                        num_vars:1,
            ...
            expected_constants: vec![
                Object::CompiledFunction(
                    CompiledFunctionStruct{
                        instructions: join_instructions(
                            vec![
                                make(code::Opcode::OpGetFree, vec![0]).unwrap(),
                                make(code::Opcode::OpGetFree, vec![1]).unwrap(),
                                make(OpAdd, vec![]).unwrap(),
                                make(code::Opcode::OpGetLocal, vec![0]).unwrap(),
                                make(OpAdd, vec![]).unwrap(),
                                make(code::Opcode::OpReturnValue, vec![]).unwrap()
                ...
                Object::CompiledFunction(
                    CompiledFunctionStruct{
                        instructions: join_instructions(vec![
                            make(code::Opcode::OpGetFree, vec![0]).unwrap(),
                            make(code::Opcode::OpGetLocal, vec![0]).unwrap(),
                            make(code::Opcode::OpClosure, vec![0, 2]).unwrap(),
                            make(code::Opcode::OpReturnValue, vec![]).unwrap()  
                        ]),
                        num_args: 1,
                        num_vars: 2
                ...
                Object::CompiledFunction(
                    CompiledFunctionStruct {
                        instructions: join_instructions(vec![
                            make(code::Opcode::OpGetLocal, vec![0]).unwrap(),
                            make(code::Opcode::OpClosure, vec![1,1]).unwrap(),
                            make(code::Opcode::OpReturnValue, vec![]).unwrap()
                        ]),
                        num_args:1,
                        num_vars:2
