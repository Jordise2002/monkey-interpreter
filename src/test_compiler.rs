...
fn test_instructions(actual: Instructions, expected: Vec<Instructions>) {
    ...
    assert_eq!(actual.content.len(), expected.content.len(), "wrong length got {:?} wanted {:?}", actual.content, expected.content);
...
fn test_integer_arithmetic() {
    let tests = vec![
        CompilerTestCase {
            ...
            expected_instructions: vec![make(OpConstant, vec![0]).unwrap(), make(OpConstant, vec![1]).unwrap()]
        }
    ];
